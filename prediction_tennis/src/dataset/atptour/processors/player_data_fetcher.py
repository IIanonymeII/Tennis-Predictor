...
class PlayerDataFetcher:
    ...
    def fetch_multiple_players_data(
        ...
        for original_player_name in tqdm(player_names, desc="Fetching player data", unit="players"):
            try:
                ...
                self.logger.debug(
                    f"Successfully processed '{original_player_name}': found {len(player_entries)} unique entries"
                )
    ...
    def _process_single_player(self, original_player_name: str) -> List[Dict[str, Optional[str]]]:
        """
    ...
    def _deduplicate_player_entries(
        self, new_data: List[Dict[str, Optional[str]]], seen_player_ids: Set[Optional[str]]

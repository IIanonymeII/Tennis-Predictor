...
from prediction_tennis.src.dataset.atptour.utils.player_utils import build_player_url, clean_key, extract_names, find_best_match
...
def _process_single_player_match(original_player_name: str,
    ...
    try:
        ...
        best_match_name, match_score = find_best_match(target= cleaned_player_name,
                                                       names = atp_player_names)
        ...
        if match_score >= minimum_match_score:
            ...
            logger.debug(f"✅ Matched '{cleaned_player_name}' to '{best_match_name}' (score: {match_score:.1f}, ID: {matched_player_id})")
            return "success"
            
        else:
            # Log failed match with context
            player_ids = players_dataframe.loc[players_dataframe['player_name'] == original_player_name, 'player_id'].values
            
            context_player_id = player_ids[0] if len(player_ids) > 0 else 'N/A'
            
            logger.warning(f"❌ Low match score for '{cleaned_player_name}' "
...
def process_players_data_with_matching(players_dataframe: pd.DataFrame,
                                       fetched_player_data: Dict[str, List[Dict[str, Optional[str]]]]
                                       ) -> pd.DataFrame:
    """
    ...
    for original_player_name in tqdm(fetched_player_data.keys(),
                                     desc="Processing player matches",
                                     unit="players"
                                    ):
        try:
            match_result = _process_single_player_match(original_player_name=original_player_name,
    ...
    logger.info(f"Player processing complete:\n"
...
def _fetch_detailed_player_data(atp_url: str,
...
def _parse_player_data_to_object(raw_player_data: Dict,
    ...
    try:
        ...
        player_object = parse_player_json(data=raw_player_data,
...
def collect_detailed_player_objects(players_dataframe: pd.DataFrame) -> List[Player]:
    ...
    for player_row in tqdm(
        ...
        try:
            # Fetch detailed player data from ATP URL
            detailed_player_data = _fetch_detailed_player_data(atp_url=player_row.url_atptour,
            ...
            player_object = _parse_player_data_to_object(raw_player_data=detailed_player_data,
            ...
            logger.error(f"Failed to process detailed data for player '{player_row.player_name}': {exc}")
            failed_fetches += 1
    
    # Log collection summary
    success_rate = (len(successfully_parsed_players) / total_players_to_process * 100
                    if total_players_to_process > 0 else 0)
    
    logger.info(f"Player object collection complete:\n"

...
from prediction_tennis.src.dataset.flashscore.models.tournaments import (
    Tournaments,
    TournamentsMinimaliste,
)
from prediction_tennis.src.dataset.flashscore.models.matchs import Match
from prediction_tennis.src.dataset.flashscore.parsers.match_status_parser import (
    FlashscoreMatchStatusProcessor,
)
from prediction_tennis.src.dataset.flashscore.parsers.matchs_in_tournament_parser import (
    FlashscoreMatchInTournamentParser,
)
from prediction_tennis.src.dataset.flashscore.parsers.odds_parser import FlashscoreOddsParser
from prediction_tennis.src.dataset.flashscore.parsers.match_score_parser import (
    FlashscoreMatchScoreProcessor,
)
from prediction_tennis.src.dataset.flashscore.parsers.tournament_dates_parser import (
    FlashscoreTournamentArchiveParser,
)
from prediction_tennis.src.dataset.flashscore.parsers.tournaments_parser import (
    FlashscoreTournamentProcessor,
)
...
ALREADY_DONE = [
    ...
    "marseille",
    "mediterranean-games",
    "melbourne-great-ocean-road-open",
    "melbourne-murray-river-open",
...
def main():
    ...
    tourn_global_list: List[TournamentsMinimaliste] = parser_global_tournament.process_data(
        data_str=response_txt
    )
    ...
    for tournament_minimaliste in minimal_tournament_bar:
        ...
        if minimal_slug in (ALREADY_DONE + EXCLUDED_GROUPS):
            continue
        # Initialize an empty list to accumulate tournament dictionaries
        tournament_dicts: List[Dict[str, str]] = []

        tournaments_list: List[Tournaments] = parser_tournament.found_all_date_in_archive(
            tournament=tournament_minimaliste
        )
        ...
        for tournament in tournament_bar:
            ...
            match_list: list[Match] = parser_match_in_tournament.find_all_matches_in_tournament(
                tournament=tournament
            )

            # Process each match in the tournament
            match_bar = tqdm(
                match_list, desc=f"Processing {tournament_name} match: Unknown", leave=False
            )
        ...
        save_tournament_data_to_csv(
            tournament_data=tournament_dicts,

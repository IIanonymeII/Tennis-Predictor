...
def handle_si(value: str):
    ...
    if current_stat is not None and sh_value is not None:
        ...
        stats_dict[last_event][last_feature][current_stat] = {"player_1": sh_value, "player_2": si_value}
...
def process_segment(segment: str):
    ...
    if clean_prefix in prefix_handlers:
        ...
        raise ValueError(f"Unrecognized prefix '{prefix}'. Valid prefixes are: {list(prefix_handlers.keys())}")

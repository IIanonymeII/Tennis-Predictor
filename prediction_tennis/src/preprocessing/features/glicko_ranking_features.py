...
def compute_glicko_movement(matches_df: pd.DataFrame,
                            lookback_matches: int = 5) -> np.ndarray:
    """
    Calculate Glicko rating movement for each match based on historical ratings.
    
    This function computes how much a player's rating has changed compared to
    their rating from a specified number of matches ago. This can be useful
    for understanding recent form and momentum.
    ...
    Args:
        matches_df: DataFrame containing match details with required columns:
            ...
            - 'player2_id_factor': Numeric ID for player 2
            - 'match_date': Date of the match
            - 'glicko_p1': Pre-match Glicko rating for player 1
            - 'glicko_p2': Pre-match Glicko rating for player 2
        lookback_matches: Number of matches to look back for comparison
    
    Returns:
        2D numpy array of shape (n_matches, 2) where each row contains
        the rating movement for [player1, player2]
    ...
    Example:
        >>> matches_df = pd.DataFrame({
        ...     'player1_id_factor': [0, 0],
        ...     'player2_id_factor': [1, 1], 
        ...     'match_date': ['2023-01-01', '2023-01-15'],
        ...     'glicko_p1': [1500, 1520],
        ...     'glicko_p2': [1480, 1460]
        ... })
        >>> movements = compute_glicko_movement(matches_df, lookback_matches=1)
        >>> movements.shape
        (2, 2)
    """
    logger.info(f"Computing Glicko movement for {len(matches_df)} matches with {lookback_matches} match lookback")
    
    if lookback_matches <= 0: raise ValueError("lookback_matches must be positive")
    
    # Validate required columns
    required_columns = ['player1_id_factor', 'player2_id_factor', 'match_date', 'glicko_p1', 'glicko_p2']
    missing_columns  = [col for col in required_columns if col not in matches_df.columns]
    if missing_columns: raise ValueError(f"Missing required columns: {missing_columns}")
    ...
    logger.debug(f"Total players for movement calculation: {total_players}")
    ...
    # Initialize array to store movement values
    match_movements = np.zeros((len(matches_df), 2), dtype=float)
    
    # Set up progress bar
    progress_description = f"Calculating last {lookback_matches} Glicko movements"
    progress_bar = tqdm(matches_df.itertuples(), total=len(matches_df), desc=progress_description)
    ...
    for match_row in progress_bar:
        ...
        player2_id = match_row.player2_id_factor
        match_date = match_row.match_date
        pre_match_rating_p1 = match_row.glicko_p1
        pre_match_rating_p2 = match_row.glicko_p2
        ...
        movement_player1 = calculate_rating_movement_from_history(
                            ...
                            current_pre_match_rating = pre_match_rating_p1,
        ...
        movement_player2 = calculate_rating_movement_from_history(
                            ...
                            current_pre_match_rating = pre_match_rating_p2,
                            matches_lookback         = lookback_matches)
        ...
        player_rating_histories[player1_id].append((match_date, pre_match_rating_p1))
        player_rating_histories[player2_id].append((match_date, pre_match_rating_p2))
    
    logger.info(f"Completed Glicko movement computation for {len(matches_df)} matches")

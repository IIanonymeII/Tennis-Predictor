...
def compute_simple_ranking(
    ...
    update_win, update_lose = _calculate_update_values(
        win_step, lose_step, transformation_method, transformation_factor
    )
    ...
    # Process each match
    progress_bar = tqdm(
        matches_df.itertuples(), total=len(matches_df), desc="Processing simple ranking"
    )
...
def compute_transformed_winloss_rankings(
    matches_df: pd.DataFrame,
    surface: str = "all",
    win_step: float = DEFAULT_WIN_STEP,
    lose_step: float = DEFAULT_LOSE_STEP,
    ...
    progress_bar = tqdm(
        ...
        desc="Computing transformed win-loss rankings",
    )
    ...
    for row in progress_bar:
        ...
        player1_rating = _apply_transformation(
            win_counts[player1_id], win_transform_method
        ) - _apply_transformation(lose_counts[player1_id], lose_transform_method)
        player2_rating = _apply_transformation(
            win_counts[player2_id], win_transform_method

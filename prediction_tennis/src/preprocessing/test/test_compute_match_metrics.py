...
# === [SIMPLE COUNT PLAYED] TEST TRUE VALUE ===
def test_compute_match_count_ratios_valid_inputs(sample_matches_df: pd.DataFrame) -> None:
...
# === [SIMPLE COUNT WIN] TEST TRUE VALUE ===
def test_compute_win_count_ratios_valid_inputs(sample_matches_df: pd.DataFrame) -> None:
    ...
    # Compute player popularity ratings using the function under test
    result: np.ndarray = compute_win_count_ratios(
        matches_df=sample_matches_df,
        period_days=period_days,
        use_ratio=use_ratio,
    )
...
@pytest.mark.parametrize("metric_function", [compute_match_count_ratios, compute_win_count_ratios])
...
def test_compute_match_input_value(
    ...
    if should_raise:
        with pytest.raises((ValueError, TypeError)):
            metric_function(sample_matches_df, **kwargs)
    else:
        result = metric_function(sample_matches_df, **kwargs)
...
if __name__ == "__main__":
    ...
    test_compute_win_count_ratios_valid_inputs(sample_matches_df=df_test)